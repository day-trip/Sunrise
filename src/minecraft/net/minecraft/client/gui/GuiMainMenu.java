...
import com.daytrip.sunrise.gui.GuiScreenProfile;
...
public class GuiMainMenu extends GuiScreen {
     ...
     */
    public void initGui() {
        buttonList.add(new GuiIconButtonClose(0, width - 20 - 3, height - 20 - 7));

        buttonList.add(new GuiIconButtonSettings(1, 30, height - 20 - 7));
        buttonList.add(new GuiButtonLanguage(2, 3, height - 20 - 7));

        buttonList.add(new GuiButton(3, width / 2 - 50 - 40, height - 20 - 7, 80, 20, I18n.format("menu.singleplayer")).setSmoothFont());
        buttonList.add(new GuiButton(4, width / 2 + 50 - 40, height - 20 - 7, 80, 20, I18n.format("menu.multiplayer")).setSmoothFont());

        buttonList.add(new GuiButton(5, 3, 3, 50, 20, "Profile"));
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if(button.id == 5) {
            mc.displayGuiScreen(new GuiScreenProfile(this));
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        drawDefaultBackground(0);

        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        mc.getTextureManager().bindTexture(new ResourceLocation("textures/gui/title/sun.png"));
        GlStateManager.enableBlend();
        drawModalRectWithCustomSizedTexture(width / 2 - 100, (int) InterpolationMath.linearInterpolate(height / 2f - 100, height - 100, startTicks / 30f), 0, 0, 200, 200, 200, 200);
