...
public class GuiStreamUnavailable extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        {
            field_152323_r.addAll(fontRendererObj.listFormattedStringToWidth(field_152326_h.func_152561_a().getFormattedText(), (int)((float) width * 0.75F)));

            if (field_152327_i != null)
            {
                field_152323_r.add("");

                for (ChatComponentTranslation chatcomponenttranslation : field_152327_i)
                {
        ...
        {
            buttonList.add(new GuiButton(0, width / 2 - 155, height - 50, 150, 20, I18n.format("gui.cancel")));
            buttonList.add(new GuiButton(1, width / 2 - 155 + 160, height - 50, 150, 20, I18n.format(field_152326_h.func_152559_b().getFormattedText())));
        }
        else
        {
            buttonList.add(new GuiButton(0, width / 2 - 75, height - 50, 150, 20, I18n.format("gui.cancel")));
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        int i = Math.max((int)((double) height * 0.85D / 2.0D - (double)((float)(field_152323_r.size() * fontRendererObj.FONT_HEIGHT) / 2.0F)), 50);
        drawCenteredString(fontRendererObj, field_152324_f.getFormattedText(), width / 2, i - fontRendererObj.FONT_HEIGHT * 2, 16777215);

        for (String s : field_152323_r)
        {
            drawCenteredString(fontRendererObj, s, width / 2, i, 10526880);
